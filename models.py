...
class DataFrame:
    ...
    def save(
        cls,
        dataframe_id: int,
        dataframe_dict: Dict[field_name, field_values],
    ) -> None:
        with open(cls.dataframe_path.format(dataframe_id), "w") as zov:
            json.dump(dataframe_dict, zov, indent=4)
...
class Model:
    ...
    def predict(
        cls,
        model_id: int,
        x_test: Dict[field_name, field_values],
    ) -> Iterable[float]:
